...
const init = (urlRoot = "") => {
  ...
  function drawMap() {
    ...
    d3.json(`${urlRoot}/geojson/world_countries.json`, function(error, world) {
      if (error) throw error;

      d3.json(`${urlRoot}/geojson/our_cities.geojson`, function(error, cities) {
        if (error) throw error;
  ...
  const loadData = function(cb) {
    if (!data[selectedAttribute]) {
      d3.json(`${urlRoot}/data/cityApp_attributes_consolidated_${selectedAttribute}.json`, function(err, filedata) {
